...
pub mod reactive_runner_plugin {
    ...
    #[cfg(test)]
    mod tests {
        use crate::bevy_tick_counter::{TickCount, TickPlugin};
...
pub mod bevy_matrix_api {
    ...
    use crate::reactive_runner_plugin::ReactiveRunner;
    ...
    pub trait MakeReq {
        async fn make_req<T: Bundle, R: Send + Sync + 'static>(
            &self,
            req: T,
        ) -> async_channel::Receiver<R>;
        fn make_req_blocking<T: Bundle, R: Send + Sync + 'static>(
            &self,
            req: T,
        ) -> async_channel::Receiver<R>;
    }
    ...
    impl MakeReq for ReactiveRunner {
        async fn make_req<T: Bundle, R: Send + Sync + 'static>(
            ...
            self.send_fn(move |app| {
                ...
                commands.spawn((APITx::new(tx), req));
            })
            .await;
            rx
        }
        ...
        fn make_req_blocking<T: Bundle, R: Send + Sync + 'static>(
            ...
            self.send_fn_blocking(move |app| {
                ...
                let mut commands = world.commands();
                trace!("SPAWNING");
                commands.spawn((APITx::<R>::new(tx), req));
            });
            rx
        }
    }
    ...
    mod tests {
        ...
        fn make_req() {
            ...
            let request_rx = reactive_runner.make_req_blocking::<Foo, ()>(Foo);
            ...
            // tick(&mut app, &runner_rx);
        }
    }

    // impl MakeReq for ReactiveRunner {
    //     fn make_req(req: Self::Input) -> async_channel::Sender<Self::Output> {}
    // }
...
pub mod bevy_matrix_reqwest {
    ...
    use crate::bevy_tokio::AsyncQueue;
...
pub mod bevy_matrix_versions {
    ...
    // pub trait MakeReqMatrixVersions {
    //     async fn req_versions(&self) -> mpsc::UnboundedReceiver<ResMatrixVersions>;
    // }

    // impl MakeReqMatrixVersions for ReactiveRunner {
    //     async fn req_versions(&self) -> mpsc::UnboundedReceiver<ResMatrixVersions> {
    //         let (tx, rx) = mpsc::unbounded_channel::<ResMatrixVersions>();
    //         self.send_fn(move |app| {
    //             let world = app.world_mut();
    //             let mut commands = world.commands();
    //             commands.spawn(ReqMatrixVersions(tx));
    //         })
    //         .await;
    //         rx
    //     }
    ...
    // pub struct ReqMatrixVersions(pub mpsc::UnboundedSender<ResMatrixVersions>);
    #[derive(Debug, Component, Clone)]
    ...
    // #[derive(Debug, Component, Clone, Error)]
    // pub enum ExecutedErr {
    ...
    // #[derive(Debug, Component, Clone)]
    // pub struct MatrixVersionsHttpReqLabel(pub Entity);

    // impl Deref for MatrixVersionsHttpReqLabel {
    //     type Target = ;
    // }
    ...
    pub fn matrix_versions_executor(
        requests: Query<
            (Entity, &MatrixVersionsParams, &APITx<MatrixVersionsProgess>),
        ...
        for (entity, _params, api_tx) in requests {
            ...
            // async_queue.spawn(api_tx.send(MatrixVersionsProgess::WaitingForResponse));
            // commands.spawn((HttpReq, MatrixVersionsHttpReqLabel));
            // commands.spawn(None);
        }
    }

    pub fn matrix_versions_finish(
        responses: Query<(Entity, &HttpRes, &APITx<MatrixVersionsProgess>)>,
        mut commands: Commands,
    ) {
        for (entity, res, tx) in responses {
            ...
            let res = match res.0.as_ref() {
                ...
                Err(err) => {
                    ...
                    tx.send_blocking(MatrixVersionsProgess::Completed(Err(CompletedErr::from(
                        err.clone(),
                    ))));
                    continue;
                }
            ...
            let res: MatrixVersions = match serde_json::from_slice(res) {
                ...
                Err(err) => {
                    ...
                    tx.send_blocking(MatrixVersionsProgess::Completed(Err(
    ...
    mod tests {
        ...
        use crate::{
            ...
            bevy_matrix_reqwest::{HttpReq, HttpRes, MatrixReqwestPlugin},
            bevy_matrix_versions::{
                MatrixVersions, MatrixVersionsParams, MatrixVersionsPlugin, MatrixVersionsProgess,
            },
            bevy_tokio::TokioPlugin,
            reactive_runner_plugin::{ReactiveRunnerPlugin, tick_blocking},
        };
        ...
        fn req_versions() {
            ...
            // let (tx, rx) = async_channel::unbounded::<MatrixVersionsParams>();
            // reactive_runner.send_fn_blocking(move |app| {
            //     let world = app.world_mut();
            //     let mut commands = world.commands();
            //     trace!("SPAWNING");
            //     commands.spawn((APITx::<MatrixVersionsParams>::new(tx), req));
            // });

            let request_rx = reactive_runner
                .make_req_blocking::<MatrixVersionsParams, MatrixVersionsProgess>(
